 ...
 */
void __copy_rom_section(unsigned long dst, unsigned long src,
		unsigned long size) {
	...
	while (len > 0) {
		...
		if (!(src & mask_int) && !(dst & mask_int) && len >= size_int) {
			*((int *) dst) = *((int*) src);
			...
			src += size_int;
			len -= size_int;
		} else if (!(src & mask_short) && !(dst & mask_short)
				&& len >= size_short) {
			*((short *) dst) = *((short*) src);
			dst += size_short;
			src += size_short;
...
void __copy_rom_sections_to_ram(void) {
	 ...
	 */
	for (index = 0;
			__S_romp[index].Source != 0 || __S_romp[index].Target != 0
					|| __S_romp[index].Size != 0; ++index) {
		__copy_rom_section(__S_romp[index].Target, __S_romp[index].Source,
				__S_romp[index].Size);
